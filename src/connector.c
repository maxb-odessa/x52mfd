...
void *joy_connector(void *arg) {
    ...
    if (rc != LIBX52_SUCCESS) {
        ...
        plog("opening joystick for writing\n");
    ...
    if (rc != LIBX52IO_SUCCESS) {
        plog("connector: libx52 read init failed: %s\n", libx52io_strerror(rc));
        // we can't go on as well as other threads
        ctx->done = 1;
    } else {
        // real connection will be done in the loop below
        plog("opening joystick for reading\n");
    ...
    while (! ctx->done) {
        ...
        if (ctx->x52dev_ok && ! libx52_is_connected(ctx->x52dev)) {
            plog("connector: joystick is disconnected for writing\n");
            ctx->x52dev_ok = 0;
        }
        ...
        if (! ctx->x52dev_ok) {
            ...
            if (rc != LIBX52_SUCCESS) {
                ...
                ctx->x52dev_ok = 1;
                //plog("connector: connected to x52pro joystick\n");
            }
            pthread_mutex_unlock(&ctx->mutex);
        }
        ...
        if (ctx->x52ctx_ok && ! libx52io_get_vendor_id(ctx->x52ctx)) {
            plog("connector: joystick is disconnected for reading\n");
            ctx->x52ctx_ok = 0;
        }
